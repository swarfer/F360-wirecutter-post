...
longDescription = "rcKeith Grbl foam cutting - UZ follows XY for simple shapes. .";
...
var zOutput = createVariable({prefix:"U"}, xyuzFormat);
var uOutput = createVariable({prefix:"V"}, xyuzFormat);
...
function onOpen() {
  ...
  writeln("(rcKeith did this you have been warned)");
...
function onRapid(_x, _y, _z) {
  ...
  var u = uOutput.format(_x);
  var z = zOutput.format(_y);
  if (x || y ) {
...
function onLinear(_x, _y, _z, feed) {
  ...
  if (x || y) {
    if (pendingRadiusCompensation >= 0) {
      ...
      //writeln("Debug Z=" + z)
      forceXYZ(); // Always ouput even in value doesn't change from previous line KH Jan 2021
      writeBlock(gMotionModal.format(1), x, y, u , z, f);
    }
  } else if (f) {
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      ...
      writeBlock(gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), uOutput.format(u), zOutput.format(z), iOutput.format(cx - start.x), jOutput.format(cy - start.y), feedOutput.format(feed));
      break;
    default:
      linearize(tolerance);
    }
